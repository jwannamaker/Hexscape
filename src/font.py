...
class Font:
    def __init__(self, size_x, size_y, window_width, window_height, batch, group):
        self.text = []
        self.size_x = size_x
        self.size_y = size_y
        self.border_boxes = []
    ...
    def write(self, text, start_x, start_y):
        ...
        for char in text:
            if char != ' ':
                ...
                letter_sprite = pyglet.sprite.Sprite(img=letter_img, x=x, y=y, batch=self.batch, group=self.group)
                ...
                self.border_boxes.append(pyglet.shapes.Box(x, y, self.size_x, self.size_y, 
                                                           color=(0, 0, 0, 0), group=self.group))
